<template>
  <div class="sanLanMingXiZhangCatalog">
...
export default {
  ...
  methods: {
    ...
    // 增行
    addLine () {
      this.tableData.push({ subject_name: '', second_subject_name: '', page: '' })
    },
    // 删行 有值清空数据，没值删除行
    removeLine (index) {
    ...
    saveAnswer () {
      if (this.index === 0) {
        // 保存三栏明细账
        this.$store.commit('SAVE_SAN_LAN_MING_XI_ANSWER', this.tableData)
        this.$message.success('保存成功')
        console.log(this.$store.state.answer)
      }
      if (this.index === 2) {
        // 保存增值税明细账
        this.$store.commit('SAVE_SHU_LIANG_MING_XI_ANSWER', this.tableData)
        this.$message.success('保存成功')
        console.log(this.$store.state.answer)
      }
    }
