<template>
  <div class="ReferZongFenLeiZhangDetail">
    <div class="title">
      <span>总分类账</span>
      <span class="page font-12">
        分页：<span class="dis-inline w-60 normal">{{ initData.page }}</span>
        总页：<span class="dis-inline w-60 normal">{{ initData.pageAll }}</span>
      </span>
    </div>
    <div class="text-left mt-15 font-13 flex flex-align-center">
      <span class="bold">科目：</span><span class="w-160 dis-inline">{{ initData.subject_name }}</span>
    </div>
    <div class="content">
      <!--头-->
      <div class="content-header flex">
        ...
        <div class="w-70 flex flex-column">
          <div class="h-30 line-height-30 border-right-bottom-grey flex flex-justify-space-between pl-10 pr-10">
            <span class="dis-inline w-50">{{ initData.year }}</span><span>年</span>
          </div>
...
<script>
export default {
  name: 'ReferZongFenLeiZhangDetail',
  data () {
    return {
      initData: {
        page: '',
  ...
  created () {
    // 初始数据
    let answer = this.$store.state['answer']['zongFenLeiZhang']['answer']
